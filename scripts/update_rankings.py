...
def prior(snapshots,name,days):
    ...
    # 3-day metric: nearest snapshot within 1 day.
    if days == 3:
        return item if distance <= 1 else None

    if distance <= max(2,days//2):
        return item
...
def table(items):
    ...
    for i,project in enumerate(items[:10],1):
        gain_3d=project.get("gain_3d")
        growth_3d=project.get("growth_3d")

        gain_text="—" if gain_3d is None else f"{gain_3d:+,}"
        growth_text="—" if growth_3d is None else f"{growth_3d:+.1f}%"
...
def update_readme(projects):
    popular=sorted(
        projects,
        key=lambda p:(-p["popularity_score"],-p["stars"])
    )

    trending=[
        p for p in projects
        if (p.get("gain_3d") or 0)>0
    ]

    trending.sort(
        key=lambda p:(
            -p["gain_3d"],
            -(p.get("growth_3d") or 0),
            -p["stars"]
        )
    )
    ...
    # Clean legacy visible wording if present.
    text=text.replace("7-day growth","3-day growth")
    text=text.replace("7d Growth","3d Growth")
    text=text.replace("7d Gain","3d Gain")
...
def main():
    ...
    print(
        f"[1/4] Updating GitHub metrics for {len(pool)} repositories...",
        flush=True
    )
    ...
    for index,entry in enumerate(pool,1):
        ...
        try:
            ...
            previous_3d=prior(snapshots,name,3)
            ...
            projects.append({
                ...
                "gain_3d":gain(stars,previous_3d),
                "growth_3d":pct(stars,previous_3d),
        ...
        if index%25==0 or index==len(pool):
            print(
                f"  {index}/{len(pool)} complete; failures={len(failures)}",
                flush=True
            )
    ...
    max_gain=max(
        [max(p.get("gain_3d") or 0,0) for p in projects] or [1]
    )

    for project in projects:
        star_score=math.log1p(project["stars"])/math.log1p(max_stars)

        if project.get("gain_3d") is None:
            popularity=star_score
        else:
            gain_score=(
                math.log1p(max(project["gain_3d"],0))/math.log1p(max_gain)
                if max_gain>0 else 0
            )
            popularity=0.70*star_score+0.30*gain_score

        project["popularity_score"]=round(popularity*100,2)

        growth=max(project.get("growth_3d") or 0,0)
        star_gain=max(project.get("gain_3d") or 0,0)

        project["rising_score"]=round(
            math.log1p(star_gain)*growth,
            3
        )
    ...
    print(
        f"Done. Updated {len(projects)} repositories; failures={len(failures)}.",
        flush=True
    )
